...
LATEX_CMD_DOCS: dict[str, tuple[str, str, str, str]] = {
    ...
    "tabular": (
        ...
        "Ambiente tabella di base. Le <b>specifiche colonne</b> definiscono allineamento "
        "e bordi: <b>l</b> (sinistra), <b>c</b> (centro), <b>r</b> (destra), "
        "<b>p{largh}</b> (paragrafo), <b>|</b> (bordo verticale). "
        "Celle separate da &amp;, righe chiuse con \\\\. "
        "Per tabelle di qualità professionale usare il pacchetto <b>booktabs</b>.",
        "LaTeX base",
    ...
    "tabularx": (
        ...
        "Come tabular ma con larghezza totale fissa. "
        "Il tipo di colonna <b>X</b> si espande per riempire lo spazio disponibile; "
        "più colonne X si dividono equamente la larghezza residua. "
        "Richiede il pacchetto <b>tabularx</b>.",
        "tabularx",
        r"\begin{tabularx}{\textwidth}{lXr}" + "\n"
        r"  ID & Descrizione & Valore \\" + "\n"
        r"\end{tabularx}",
    ),
    "tabulary": (
        r"\begin{tabulary}{larghezza}{specifiche}",
        "Come tabularx ma bilancia l'altezza delle righe in modo più naturale. "
        "Tipi di colonna: <b>L</b> (sinistra), <b>C</b> (centro), <b>R</b> (destra), "
        "<b>J</b> (giustificato). "
        "Richiede il pacchetto <b>tabulary</b>.",
        "tabulary",
        r"\begin{tabulary}{\textwidth}{LCR}" + "\n"
        r"  Sinistra & Centro & Destra \\" + "\n"
        r"\end{tabulary}",
    ),
    "longtable": (
        r"\begin{longtable}[pos]{specifiche}",
        "Tabella che può spaziarsi su più pagine. "
        "Le sezioni \\endfirsthead, \\endhead, \\endfoot, \\endlastfoot definiscono "
        "intestazioni/piè da ripetere su ogni pagina. "
        "Richiede il pacchetto <b>longtable</b>.",
        "longtable",
        r"\begin{longtable}{|l|c|}" + "\n"
        r"  \hline Col A & Col B \\ \hline \endhead" + "\n"
        r"  Riga 1 & Dati \\" + "\n"
        r"\end{longtable}",
    ),
    "array": (
        r"\begin{array}{specifiche}",
        "Ambiente tabella per uso matematico (in modalità math). "
        "Stesse specifiche colonne di tabular ma per formule. "
        "Richiede il pacchetto <b>array</b> per i tipi estesi.",
        "LaTeX base",
        r"$\begin{array}{ccc} a & b & c \\ d & e & f \end{array}$",
    ),
    "tabular*": (
        r"\begin{tabular*}{larghezza}[pos]{specifiche}",
        "Tabella a larghezza fissa. Lo spazio extra viene distribuito usando "
        "\\extracolsep{\\fill} nelle specifiche di colonna. "
        "Preferire tabularx per risultati più prevedibili.",
        "LaTeX base",
        r"\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lll}",
    ),
    "newcolumntype": (
        r"\newcolumntype{lettera}{definizione}",
        "Definisce un nuovo tipo di colonna per tabular/tabularx. "
        "Può includere formattazione automatica. Richiede il pacchetto <b>array</b>.",
        "array",
    ...
    "pagestyle": (
        r"\pagestyle{stile}",
        "Imposta lo stile di intestazione e piè di pagina per tutto il documento. "
        "Stili: <b>plain</b>, <b>empty</b>, <b>headings</b>, <b>myheadings</b>.",
        "LaTeX base",
        r"\pagestyle{headings}",
    ),
    "fancyhf": (
